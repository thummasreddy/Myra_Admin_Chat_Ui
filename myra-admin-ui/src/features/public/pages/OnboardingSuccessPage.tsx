    ...
    <main className="min-h-screen bg-[var(--color-bg-main)] text-white">
      ...
      <section className="public-pricing-hero border-b px-4 py-14 text-white sm:px-6 lg:px-8">
        <div className="mx-auto max-w-4xl">
          <div className="mb-5 flex h-14 w-14 items-center justify-center rounded-md bg-[var(--color-success-bg)] text-[var(--color-success)]">
        ...
        <Card className="public-card">
          ...
          <CardContent className="space-y-5">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-md border border-primary/15 bg-[var(--color-bg-card)] p-4">
  ...
  return (
    <div className="grid gap-3">
      {steps.map((step, index) => (
        <div key={step} className="flex items-center gap-3 rounded-md border bg-[var(--color-bg-card)] p-3 text-sm">
          <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-primary/10 text-xs font-semibold text-primary">
            {index + 1}
          </div>
          <span className="font-medium text-[var(--color-text-secondary)]">{step}</span>
        </div>
      ))}
    </div>
  );
